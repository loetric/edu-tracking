-- ============================================
-- CREATE file_reads TABLE FOR FILE TRACKING
-- ============================================
-- This script creates the file_reads table to track which users have read which files
...
CREATE INDEX IF NOT EXISTS idx_file_reads_read_at ON public.file_reads(read_at DESC);

-- RLS Policies for file_reads
ALTER TABLE public.file_reads ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can mark files as read" ON public.file_reads;
DROP POLICY IF EXISTS "Users can view file reads" ON public.file_reads;
DROP POLICY IF EXISTS "Admins can delete file reads" ON public.file_reads;
...
DO $$
BEGIN
    RAISE NOTICE '✅ file_reads table created successfully!';
END $$;
